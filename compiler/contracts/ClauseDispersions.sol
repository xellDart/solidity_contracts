...
// File: Payment/ClauseDispersion.sol

pragma solidity >=0.6.2;


contract ClauseDispersion is Ownable, Stage, Members, Info {
    struct Conditions {
        bytes prop_;
        bool executed_;
        address member_;
    ...
    struct Clause {
        ...
        bytes type_;
        bytes name_;
        mapping(uint256 => Conditions) conditions_;
        uint256 conditionsIndex_;
        mapping(address => Pay) pay_;
        address receiver_;
        bool executed_;
        bool withPay_;
        bool isDeleted_;
    ...
    Clause[] private clauses_;
    Escrow private _escrow;
    bool private payOnAllClauses_;
    ...
    function setOnAll(bool pay) public onlyOwner atStage(Stages.CREATION) {
        payOnAllClauses_ = pay;
    }
    ...
    function setClausePay(
        uint256 _clause,
        uint256 _quantity,
        address _receiver
    ) public onlyOwner atStage(Stages.CREATION) {
        require(clauses_[_clause].withPay_, "Clause not accept pays");
        Pay memory _pay;
        _pay.amount_ = _quantity;
        clauses_[_clause].pay_[_receiver] = _pay;
        clauses_[_clause].receiver_ = _receiver;
    }
    ...
    function addCondition(
        ...
        Conditions memory _condition = Conditions(
            _description,
            false,
            _member,
            false
        );
        uint256 index = clauses_[_clause].conditionsIndex_;
        clauses_[_clause].conditions_[index] = _condition;
        clauses_[_clause].conditionsIndex_ = index + 1;
    }

    function removeClause(uint256 _clause)
        public
        onlyOwner
        atStage(Stages.CREATION)
    {
        clauses_[_clause].isDeleted_ = true;
    }

    function removeCondition(uint256 _clause, uint256 _condition)
        public
        onlyOwner
        atStage(Stages.CREATION)
    {
        clauses_[_clause].conditions_[_condition].isDeleted_ = true;
    }

    function executeCondition(uint256 _clause, uint256 _condition)
        public
        onlyOwner
        atStage(Stages.RUNNING)
    {
        clauses_[_clause].conditions_[_condition].executed_ = true;
        setClauseExecuted(_clause);
    }

    function setClauseExecuted(uint256 _clause) private {
        uint256 index = clauses_[_clause].conditionsIndex_;
        bool _executed = false;
        for (uint8 i = 0; i < index; i++)
            _executed = clauses_[_clause].conditions_[i].executed_;
        if (_executed) {
            clauses_[_clause].executed_ = true;
            Pay memory _temp = clauses_[_clause].pay_[clauses_[_clause]
                .receiver_];
            if (clauses_[_clause].withPay_) {
                pay(clauses_[_clause].receiver_, _temp.amount_);
            }
        }
        if (payOnAllClauses_) allExecuted();
    }

    function allExecuted() private {
        bool _executed = false;
        for (uint256 i = 0; i <= clauses_.length; i++) {
            _executed = clauses_[i].executed_;
        }
        if (_executed) disperseAll();
    }
    ...
    function disperseAll() private onlyOwner atStage(Stages.RUNNING) {
...
contract Trato is ClauseDispersion {
    bytes private hash_;
    address private trato;
